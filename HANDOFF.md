...
- M2：28×15m 解析场地、50mm 地形高度图、整体 1°–2° 横坡、支持逐顶点高度的多边形表面、150mm 堡垒及六个 20° 坡面、中央高地 200–350mm/10.5° 连接坡、梯形高地 200–400mm/23°/43° 分层、17° 飞坡、70mm/240mm 起伏路、图 4-5 基地/前哨站/堡垒中心、带安全间距的八轮迭代接触约束、区域、LOS、装甲交点、可配置命中模型、部分可观测观测，以及目标优先/分角色战术两类脚本对手。
...
完整测试为 111 项；`ruff`、`mypy`、`compileall`、命令行训练/评估冒烟和原生 Isaac Lab CPU 无头场景冒烟均通过。接触与地形回归覆盖同点分离、随机批量运动、静态障碍、坡面逐顶点插值、堡垒分面和完整脚本对局。完整 420 秒 H.264 导出为 1120×700、421 帧、21.05 秒，逐策略步外廓检查的最小地面单位净间距为 0.0117m。画面按相对局部场地面的高度分层设色，带 50mm 等高色带、坡向箭头、边缘阴影和独立高度图例；阵营色只用于边界。此前的吞吐数字基于四轮接触求解，改为八轮后应在 GPU 空闲时重新基准，不沿用旧数字。
