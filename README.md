...
The replay uses one isotropic `36 px/m` transform: the `28×15 m` field occupies exactly `1008×540 px` inside a `1120×706 px` canvas, and the elevation raster is sampled at final-pixel centers. A neutral sequential palette, 50mm contour bands, slope arrows, and a height legend distinguish elevations without stretching either axis. Published dimensions are kept separate from referee regions: the base plan bounds are `1.609 m` along field x by `1.881 m` along y, and the outpost body is `Ø0.550 m` (the manual's `0.650 m` pedestal value is only a one-axis width). The aerial pad has a `2.200×2.858 m` outer structure envelope and a `2.149×2.200 m` landing octagon with `1.334 m` straight edges. The trapezoid (`10.805×4.380 m`), road (`8.901×3.651 m`), fortress (`2.240×1.939 m`), fly ramp (`1.145×0.860 m`), and rough-road dimensions follow figures 4-25–4-37. Placements or internal subdivisions that the PDF does not uniquely dimension remain marked `[SIM]` or `[AMB]`; the scene is not official CAD. The exporter rejects any ground-footprint overlap. Use `--steps 300` only for a short smoke export.
