...
The replay uses one isotropic `36 px/m` transform: the `28×15 m` field occupies exactly `1008×540 px` inside a `1120×706 px` canvas, and the elevation raster is sampled at final-pixel centers. A neutral sequential palette, 50mm contour bands, slope arrows, and a height legend distinguish elevations without stretching either axis. Geometry follows the manual's dimension chains rather than treating every labelled span as a bounding box. The base uses a `1.609×1.881 m` dimension-chain core approximation; its front edge is `[DIGITIZED]`, peripheral details are omitted, and the axial chain resolves a small overconstraint in the drawing. The outpost's `Ø0.550 m` value is the rotating-armor sweep envelope, not a solid body; `0.650 m` is only a one-axis pedestal width. Figure 4-16 publishes aerial-pad frame spans of `2200/2149/1334/2200/2858 mm`, but does not establish a closed outer rectangle or fully dimension the landing platform and its offset. The trapezoid uses a `200 mm` main deck, `400 mm` gain platform, and `23°/43°` edges—`350/600 mm` are guard heights. Figure 4-34 constrains the road station chain, but `8901×3651 mm` is not a complete polygon; the `11°/15°` faces remain a dashed `[AMB]` reference rather than a false flat collision mesh. Assembly `100/200 mm` levels are handled the same way because their face-to-plan mapping is incomplete. The tunnel is likewise only a `[SIM/AMB]` plan reference: its published width and section dimensions do not fix axis length, pose, or roof interpretation. The fly ramp itself is dimensioned, while its opposite landing is represented only by the published `1.143 m` transverse edge and `0.650 m` gap because platform depth is absent. Only the rough-road `70 mm` bump height and `240 mm` pitch are official. Unlabelled world transforms or closures remain `[DIGITIZED]`, `[INFERRED]`, `[AMB]`, or `[SIM]`; the scene is not official CAD. Use `--steps 300` only for a short smoke export.
...
Add `--device cpu` when CUDA simulation is unavailable or GPU memory is occupied. The Isaac scene runs the same tactical controller, polygonal terrain footprints, per-vertex slope elevations, and authoritative Torch game state; non-rectangular surfaces are deterministically triangulated into meshes rather than replaced by bounding boxes. Every polygon-mesh vertex includes the crowned-field height at its world coordinate, matching the Torch terrain surface instead of applying one center-height offset. The rough road instantiates the published 70mm/240mm bump profile on a `[SIM]` strip footprint. It does not introduce a second gameplay state.
