...
第一批纵切已经落地：`rm_referee` 状态/输入/事件契约，战斗、热量、功率、弹量购买、读条复活、雷达递推、建筑胜负模块，`RandomTape`，轻量 Torch 运动学，以及 Isaac 几何归一化桥。规则与后端契约测试共 27 项；M1 尚未完成，角色特殊机制、升级、能量机关、飞镖和完整雷达计时仍在后续清单中。
...
`ARMOR_REFRACTORY_S` 只过滤同一装甲模块接受的伤害，不能限制射击、热量或弹量。热量永久锁定固定使用流程图语义 `Q1 >= Q2`，不实现兼容开关。
