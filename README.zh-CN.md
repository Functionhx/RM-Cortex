...
回放统一使用等比例 `36 px/m` 变换：`28×15 m` 场地严格占据 `1008×540 px`，外层画布为 `1120×706 px`，高度栅格直接在最终像素中心采样。几何按工程图尺寸链建模，不再把每个标注跨度误当外接矩形。基地采用 `1.609×1.881 m` 尺寸链核心近似：前端边缘为 `[DIGITIZED]`，省略两侧附属细节，并以轴向尺寸链消解图纸中的轻微过约束。前哨站 `Ø0.550 m` 是三块旋转装甲的扫掠包络而非实体直径，`0.650 m` 只是底座单向宽度。图 4-16 可靠给出停机坪框架的 `2200/2149/1334/2200/2858 mm` 跨度，但没有建立封闭外矩形，也没有完整标注中央平台及其偏移。梯形高地主行驶面为 `200 mm`、小 R 增益台为 `400 mm`，边坡为 `23°/43°`；`350/600 mm` 是护墙高度。图 4-34 约束了公路站距链，但 `8901×3651 mm` 不是完整多边形；`11°/15°` 坡面保留为虚线 `[AMB]` 参考，不再生成错误的全平碰撞面。装配区 `100/200 mm` 高程也因分面到平面的映射不完整而采用同样处理。隧道同样只保留 `[SIM/AMB]` 平面参考：已发布的宽度与截面尺寸不足以确定轴长、姿态或屋顶语义。飞坡本体采用发布尺寸；对岸平台仅显示已知的 `1.143 m` 横向边与 `0.650 m` 间隙，因为平台纵深未标注。起伏路只有 `70 mm` 凸起高度和 `240 mm` 节距是官方尺寸。未标注的世界变换或闭合边分别标记 `[DIGITIZED]`、`[INFERRED]`、`[AMB]` 或 `[SIM]`，当前场景不是官方 CAD。仅需短时冒烟时可追加 `--steps 300`。
...
CUDA 仿真不可用或显存被占用时可追加 `--device cpu`。Isaac 场景运行同一战术控制器、同一组多边形 footprint、逐顶点坡面高度与权威 Torch 比赛状态；非矩形表面会确定性三角剖分为网格而不是退化为外接矩形，每个网格顶点还会叠加其世界坐标处的场地横坡高度，与 Torch 地形表面保持一致。起伏路在 `[SIM]` 条带范围内按官方 `240 mm` 节距生成 `70 mm` 凸起，不会引入第二套规则状态。
